...
$(function () {
  ...
  const USER_ID = '17841403377586680';
  ...
  window.Instagram.getUserRecentMedia(USER_ID, function (media) {
    ...
    filteredTags = releventTags(sortedTags, 2); //declared function below
  ...
  function releventTags (tags, minFrequency) {

    filtered = tags.filter(function (item) { return item.frequency >= minFrequency; });
    return filtered;
  ...
  function extractTagsFromMedia (media) {
    ...
    let tags = Object.keys(tagFrequency).map(function (tagId) {
      return {
        id: tagId,
  ...
  function sortTagsByFrequencyDesc (tags) {
    ...
    tagCopy = tags.slice();
    tagCopy.sort((a, b) => b.frequency - a.frequency);
    return tagCopy;
  ...
  function renderUserInfo (user) {
    ...
    $('#user h1').html(user.username);
    $('#user #num-media').html(user.counts.media);
    $('#user #num-followers').html(user.counts.followed_by);
    $('#user #num-follows').html(user.counts.follows);
  ...
  function handleClickTag (event) {
    ...
    let filteredMedia = filterImages(tagId, userMedia);
  ...
  function filterImages (hashtagId, mediaObj) {

    filteredPics = mediaObj.filter(function (pic) { return pic.tags.includes(hashtagId); });
    return filteredPics;
